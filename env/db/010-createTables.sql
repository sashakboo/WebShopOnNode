...
(
    ...
    category bigint
        constraint products_productscategory_id_fk
            references productscategory,
    price    numeric,
    tag      varchar(250),
    icon     bytea,
...
create table basket
(
    id       bigint default nextval('public.basket_id_seq'::REGCLASS) not null
    ...
    product  bigint                                                   not null
        constraint basket_products_id_fk
            references products
);
...
(
    id    bigint default nextval('public.order_id_seq'::REGCLASS) not null
        constraint orderstates_pk
            primary key,
    name  varchar(250)                                            not null
        constraint orderstates_name_uindex
            unique,
    title varchar(250)                                            not null
);
